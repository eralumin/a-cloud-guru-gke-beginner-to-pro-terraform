resource "kubernetes_deployment" "myapp_isready" {
  metadata {
    name = "${var.lab_name}-myapp-isready"
    labels = {
      app = "${var.lab_name}-myapp-isready"
    }
  }
...
resource "kubernetes_service" "myapp_isready" {
  metadata {
    name = "${var.lab_name}-myapp-isready-service"
  }
  spec {
    selector = {
      app = "${var.lab_name}-myapp-isready"
